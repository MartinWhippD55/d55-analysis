...
Fetches a GitHub Pull Request, performs an iterative code review, and produces a findings summary.
...
└── summary.md    # Final findings and overall assessment
```
...
2. Present the summary to the user and ask if they'd like to post it as a PR comment:
   ```bash
   gh pr comment <number> --repo <owner/repo> --body "<summary>"
   ```
