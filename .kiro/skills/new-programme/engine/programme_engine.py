"""
Reusable branded-document engine for D55 programmes (vendored into the bundle).
...
Callers supply branding and content.

Vendored from ``analysis/D55/ai-dlc/programme_engine.py`` and generalised for the
self-sufficient skill bundle (Requirement 14):

- **Bundle-relative defaults.** ``BrandConfig`` no longer requires explicit asset
  paths; if ``logo``/``background`` are omitted it falls back to the vendored
  D55 brand assets resolved via ``engine/paths.py`` (``assets/brand/``). No
  repo-root-relative or hard-coded absolute paths anywhere.
- **Self-contained output (Requirement 8.3 / Property 12).** Images are embedded
  as base64 data URIs, and fonts default to a *system* font stack rather than a
  CDN ``@import``. The reference engine imported Inter from Google Fonts, which
  is an external dependency and would break the "no CDN links" guarantee. Here
  ``font_import`` defaults to ``None`` (system fonts, fully offline). A caller
  may set ``font_import`` to a data-URI stylesheet to embed a webfont, but
  setting a remote URL is discouraged and documented as breaking self-containment.
...
class BrandConfig:
    ...
    ``logo`` and ``background`` default to the vendored bundle brand assets
    (``assets/brand/``) resolved via ``paths.py`` — never a repo-relative or
    absolute path. ``output_dir`` defaults to the bundle's own ``output/`` so
    the engine can run standalone; callers normally pass a programme output dir.
    """
    logo: Path = field(default_factory=paths.default_logo)
    background: Path = field(default_factory=paths.default_background)
    output_dir: Path = field(default_factory=lambda: paths.bundle_root() / "output")
    org_name: str = "D55"
    ...
    # Self-contained by default: no external font. The CSS font stack below
    # degrades gracefully to system fonts. Set this to a *data:* URI to embed a
    # webfont; a remote URL would break self-containment (Property 12).
    font_import: str | None = None
    # System font stack used when no webfont is embedded.
...
body {{ font-family: {cfg.font_family}; color: #23232f; font-size: 11pt; line-height: 1.6; }}
...
def build_html(doc: dict, cfg: BrandConfig) -> str:
    ...
    # Self-contained by default: only emit an @import if a font source is
    # configured. A data: URI keeps the document offline; a remote URL would
    # break self-containment (Property 12), so the default is None (system fonts).
    font_import_css = f"@import url('{cfg.font_import}');\n" if cfg.font_import else ""
...
<style>
{font_import_css}{build_css(cfg, bg_uri)}
</style>
</head>
<body>
<div class="cover">
...
def build(doc: dict, cfg: BrandConfig | None = None, make_pdf: bool = True) -> Path:
    """Render a content dict to branded HTML (+ optional PDF). Returns the HTML path.

    ``cfg`` defaults to a :class:`BrandConfig` using the vendored bundle brand
    assets and the bundle's ``output/`` directory, so the engine runs standalone.
    """
    cfg = cfg or BrandConfig()
