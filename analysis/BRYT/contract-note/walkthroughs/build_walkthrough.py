"""
Reusable walkthrough document generator for the BRYT Contract Note estimates.

Each estimate provides a content module (e.g. estimate_01.py) exposing a DOC dict.
This engine renders that content into a branded, standalone HTML document with
embedded (base64) images, then Playwright renders the HTML to PDF.
...
Content model (DOC dict):
{
    ...
    "effort": "~13.5 developer days",
    "blocks": [ <block>, ... ]
}

Block types:
    {"type": "section", "heading": str, "body": [str, ...], "bullets": [str, ...]}
    {"type": "table", "heading": str, "columns": [str, ...], "rows": [[str, ...], ...], "intro": str?}
    {"type": "screens", "heading": str, "intro": str?, "screens": [
        {"image": path, "title": str, "body": [str, ...], "interactions": [str, ...], "data": [str, ...]}
    ]}
    {"type": "diagram", "heading": str, "image": path, "caption": str?, "body": [str, ...]?}
    {"type": "callout", "heading": str, "body": [str, ...]}

Usage:
    python analysis/BRYT/contract-note/walkthroughs/build_walkthrough.py estimate_01
"""
import base64
...
ROOT = Path(__file__).resolve().parents[1]  # analysis/BRYT/contract-note
ASSETS = ROOT / "assets"
...
def _resolve(path: str) -> Path:
    """Resolve an image path relative to the contract-note root."""
    p = Path(path)
    ...
    if candidate.exists():
        return candidate
    # Try relative to workspace root as a fallback
    return Path(path)
...
<div class="cover">
    ...
    <div class="cover-inner">
        <div class="eyebrow">Contract Note Rework</div>
        <h1>{esc(doc['title'])}</h1>
