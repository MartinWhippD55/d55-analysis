"""
Single source of truth for Report Builder estimate figures.

Reads the estimates spreadsheet's authoritative "Task Detail" rows directly with
openpyxl (raw numbers - not Excel's cached formula values, which go stale when a
non-Excel tool edits the file) and computes per-phase day figures. Every
generator (walkthrough, data model, presentation) imports from here rather than
hardcoding numbers, so one spreadsheet edit + regenerate propagates everywhere.
...
Usage:
    ...
    FIGURES["phase2"].total      # -> 14.5
    fmt(FIGURES["phase2"].total) # -> "14.5"
    effort_line("phase2")        # -> "~14.5 developer days (12.5 required + testing)"
"""
...
SPREADSHEET = Path(__file__).with_name("BRYT Report Builder Estimates.xlsx")
...
def _load() -> dict[str, Figure]:
    if not SPREADSHEET.exists():
        raise FileNotFoundError(
            f"Estimates spreadsheet not found: {SPREADSHEET}\n"
...
def effort_line(key: str) -> str:
    """Cover-badge effort string, e.g. '~14.5 developer days (12.5 required + testing)'."""
    f = FIGURES[key]
