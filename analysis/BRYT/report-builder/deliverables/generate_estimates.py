"""
Generate the Report Builder estimates spreadsheet from the spec task file.

Parses `.kiro/specs/report-builder/tasks.md` (top-level numbered tasks grouped
under `### Phase N — ...` headings), classifies each task, assigns a day weight,
and writes an xlsx with a Task Detail sheet (authoritative, one row per task) and
a Summary sheet (per-phase rollups via live SUMIFS/COUNTIF formulas).
...
will want to adjust. Everything downstream (figures.py, walkthrough, deck) reads
from the resulting spreadsheet, so a single edit here propagates everywhere.

Usage:
    python analysis/BRYT/report-builder/deliverables/generate_estimates.py
"""
import re
...
TASKS = REPO / ".kiro" / "specs" / "report-builder" / "tasks.md"
OUTPUT = Path(__file__).with_name("BRYT Report Builder Estimates.xlsx")

# --- Day-weighting table (days per task, by category) -----------------------
# Editable. These are the primary lever for tuning the estimate.
WEIGHTS = {
    "scaffold": 2.0,        # repo scaffold / bootstrapping
    "infrastructure": 1.5,  # CDK: DynamoDB, S3, API GW, IAM/LF role, Athena wg
    "security_core": 2.5,   # the security spine (generator, verifier, identity)
    "api_backend": 1.5,     # Lambda handlers, shared-lib types, persistence
    "assistant": 2.5,       # Bedrock Converse loop, tools, injection defence
    "frontend": 3.0,        # Angular screens / canvas / client model
    "testing": 2.0,         # property + integration + security test suites
    "integration": 1.0,     # deploy, health, observability, CI/CD, e2e sign-off
}
...
OVERRIDES = {
    "7": 3.0,    # Query_Generator - the pinning/CTE core
    "8": 3.0,    # Query_Verifier - independent pre-exec + result-set checks
    "20": 3.0,   # Assistant Converse loop
    "26": 3.0,   # Step Functions run pipeline (5 states + catch)
    "34": 8.0,   # Screens - 7 Angular screens in one task
}
...
def classify(task_id: str, text: str) -> str:
    ...
    if task_id == "1":
        return "scaffold"
    if "property test" in t or "integration test" in t or "test suite" in t:
        return "testing"
    if any(k in t for k in ("dynamodb", "s3 buckets", "api gateway", "execution role",
                            "athena workgroup", "jwt authorizer")):
        return "infrastructure"
    if any(k in t for k in ("validatedesign", "authorised_bryt_numbers", "query_generator",
                            "query_verifier", "identity &")):
        return "security_core"
    if any(k in t for k in ("assistant", "mutation tools", "validate_query", "injection")):
        return "assistant"
    if any(k in t for k in ("flow-canvas", "screens", "client `report_design`", "graph mapping")):
        return "frontend"
    if any(k in t for k in ("health/smoke", "observability", "ci/cd", "end-to-end walkthrough")):
        return "integration"
    return "api_backend"
...
def parse_tasks():
    ...
    for line in lines:
        ...
        if m:
            ...
            # strip trailing requirement tag e.g. " (R17)"
...
def main():
    ...
    ws = wb.active
    ws.title = "Summary"
    ws["A1"] = "BRYT Report Builder - Estimate Summary"
    ...
    for i, label in enumerate(phases):
        r = first + i
        # Column references into Task Detail: A=Estimate, E=Days, F=Optional
