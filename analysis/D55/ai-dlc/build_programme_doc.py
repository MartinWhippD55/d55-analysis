"""
Programme overview document generator for AI-DLC.

Modelled on the BRYT contract-note walkthrough engine: a content dict (DOC) is
rendered into branded, standalone HTML (images embedded as base64), then
Playwright renders the HTML to an A4 PDF.

Content lives in programme_doc_content.py (the DOC dict). This file is the engine.
...
ASSETS = ROOT / "assets"
LOGO = ASSETS / "logo" / "D55_LOGO_WHITE (2).png"
BG = ASSETS / "backgrounds" / "D55_TEAMS_BACKGROUND_No_LOGO.jpg"
...
# ---------------------------------------------------------------------------
# Block renderers  (body/bullet strings may contain inline HTML like <strong>)
# ---------------------------------------------------------------------------
...
def render_cards(block):
    """A set of titled cards, each with body + bullets. Good for the 4 modules / 5 principles."""
    heading = f'<h2>{esc(block["heading"])}</h2>' if block.get("heading") else ""
    intro = f'<p>{block["intro"]}</p>' if block.get("intro") else ""
    cards = ""
    for card in block["cards"]:
        ...
        body = _para_list(card.get("body"))
        bullets = _bullets(card.get("bullets"))
        cards += f'<div class="card">{tag}{title}{body}{bullets}</div>'
...
.cover::after {{
    ...
    background: linear-gradient(135deg, rgba(26,10,62,0.90) 0%, rgba(28,20,88,0.74) 50%, rgba(10,74,140,0.6) 100%);
}}
...
ul.bullets li::before {{ content: '\\203A'; position: absolute; left: 0; top: -1px; color: #5dade2; font-weight: bold; font-size: 13pt; line-height: 1.6; }}

/* Tables */
table.data-table {{ width: 100%; border-collapse: collapse; font-size: 9.5pt; margin-top: 8px; }}
table.data-table th {{ background: #1a0a3e; color: #d7ecfa; text-align: left; padding: 8px 10px; font-size: 8.5pt; text-transform: uppercase; letter-spacing: 0.4px; }}
...
/* Callout */
.callout {{ background: #f0f6fc; border-left: 4px solid #5dade2; padding: 14px 18px; border-radius: 0 4px 4px 0; }}
...
.lane-label {{ font-size: 8pt; font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; color: #0a4a8c; margin-bottom: 8px; }}
.lane-nodes {{ display: flex; flex-wrap: wrap; gap: 8px; }}
.lane-nodes .node {{ flex: 1; min-width: 110px; text-align: center; background: #fff; border: 1.5px solid #5dade2; border-radius: 5px; padding: 9px 6px; font-size: 8.5pt; font-weight: 600; color: #1a0a3e; }}
...
.pipeline .step {{ flex: 1; text-align: center; background: #1a0a3e; color: #fff; border-radius: 5px; padding: 11px 6px; font-size: 8pt; font-weight: 600; display: flex; align-items: center; justify-content: center; }}
.pipeline .step-arrow {{ color: #5dade2; font-size: 11pt; align-self: center; }}

/* Cards */
.cards {{ display: flex; flex-direction: column; gap: 12px; }}
.card {{ border: 1px solid #d8d8e6; border-left: 4px solid #5dade2; border-radius: 0 6px 6px 0; padding: 13px 16px; background: #fcfcfe; page-break-inside: avoid; }}
.card h3 {{ margin-bottom: 6px; }}
.card-tag {{ display: inline-block; background: #1a0a3e; color: #d7ecfa; font-size: 8pt; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; padding: 3px 9px; border-radius: 3px; margin-bottom: 8px; }}
...
<div class="cover">
    ...
    <div class="cover-inner">
        <div class="eyebrow">{esc(doc.get('eyebrow', 'AI-DLC'))}</div>
        <h1>{esc(doc['title'])}</h1>
...
def main():
    ...
    html = build_html(doc)
    slug = doc.get("slug", "ai-dlc-programme-overview")
