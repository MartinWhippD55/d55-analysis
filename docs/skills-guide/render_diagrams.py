"""Render every *.mmd in ./diagrams to a sibling PNG via headless Chromium + mermaid.

Standalone copy of the approach used by the decomposition-diagrams skill so the
skills guide can embed diagram images that survive the markdown-to-pdf render
(which treats a ```mermaid fence as a plain code block).
